    ...
    >
      ...
      >
        <View className="mt-4 mb-5">
          <View
            className="inline-flex self-start border px-3 py-1 rounded-full mb-3 flex-row items-center"
            style={{
        ...
        <View
          ...
          style={{
            ...
            borderColor: focused ? theme.primary : theme.cardBorder,
          }}
        >
          ...
          <TextInput
            multiline
            // No maxHeight is set above: the field always grows to fit its content
            // rather than scrolling internally. Multiline TextInput is backed by a
            // native scroll view (UITextView on iOS) that otherwise claims vertical
            // pan gestures that start inside it, so a swipe beginning on the text box
            // never reaches the outer ScrollView -- the user has to lift their finger
            // and start again outside the box. Since internal scrolling is never
            // needed here, disabling it lets every swipe propagate to the page scroll.
            scrollEnabled={false}
            value={source}
        ...
        >
          <Layers
            size={18}
            color={slides.length ? theme.onPrimary : theme.textMuted}
          />
          <Text
        ...
        <View
          className="border p-4 rounded-2xl flex-row items-start"
          style={{ backgroundColor: theme.card, borderColor: theme.cardBorder }}
        >
          <View
            className="p-2 rounded-xl mr-3"
