...
| | |
|---|---|
| Bundle id / package | `com.altixcode.slideforge` |
| App Store Connect app | `6811948490` (version 1.0, `PREPARE_FOR_SUBMISSION`) |
| In-app purchase | `com.altixcode.slideforge.removeads` — ASC id `6811948833`, state `MISSING_METADATA` |
| RevenueCat project | `projc678f427` |
| RevenueCat apps | iOS `appecd573938f` · Android `app9368e928d3` |
...
- **Interstitial**: app/preview.tsx — after the carousel is saved. Pacing lives in `adPolicy.ts`: the first completion
...
- **Tests**: 65 passing, typecheck clean.
...
2. **Write and upload the store listing.** This app has no description and no
   ...
   `Dev/scripts/store-metadata.json`; add an entry for SlideForge and upload it with
...
3. **Upload the IAP review screenshot.** The in-app purchase
   (`com.altixcode.slideforge.removeads`) is localized and priced at $3.99, and sits at
   `MISSING_METADATA` for want of one screenshot:

   ```bash
   asccli iap-review-screenshot upload --iap-id 6811948833 --file <path-to-png>
   ```
...
5. **Add `ar` and `fa`, then the RTL pass.** Deliberately left until last. The
...
2. **RevenueCat — Apple credentials.** `appecd573938f` (the App Store app in project
   `projc678f427`) has no Apple credentials, so App Store purchases cannot be
   validated. This needs an interactive Apple ID sign-in with 2FA:

   ```bash
   rc setup apple appecd573938f
   ```
...
3. **Create the GitHub repo first — there is none.** `git remote -v` is empty, so
   ...
   and nothing is backed up. Create `AltixCode/slideforge` and push, then add
   ...
   Set each with `gh secret set <KEY> --repo AltixCode/slideforge`. The RevenueCat
   public SDK keys are fetchable — `rc api GET "/projects/projc678f427/apps/appecd573938f/public_api_keys"`
   — the AdMob ones come from step 1.

4. **The legal URLs are wrong and they 404.** `src/config/legal.ts` points at
   `https://www.hushtunnel.com/legal/slideforge-privacy` — **hushtunnel.com**, a leftover from the HushTunnel
...
5. **Play Console — create the app.** `com.altixcode.slideforge` is not recognised by
...
../scripts/verify-app.sh slideforge com.altixcode.slideforge   # full build + device verification, both platforms
```
